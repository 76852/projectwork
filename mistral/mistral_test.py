...
class MemorizationTester:
    ...
    def format_prompt_for_mistral(self, prompt):
        """为Mistral模型格式化提示词"""
        # Mistral Instruct模型使用特殊的对话格式
    ...
    def save_complete_results(self):
        ...
        output_path = output_dir / f"mistral_results_{timestamp}.json"
...
if __name__ == "__main__":
    try:
        ...
        tester = MemorizationTester(
            model_path="/zhangguangyi01/Lianghongjian/Mistral_7B_Instruct_v0.2",
        ...
        print(f"失败测试: {tester.stats['failure_count']}")
        print(f"精确匹配数: {exact_matches}")
        
        if tester.stats['success_count'] > 0:
            print(f"精确匹配率: {exact_matches/tester.stats['success_count']*100:.2f}%")
        else:
            print("精确匹配率: 0.00%")
        ...
        print("\n=== 难度级别分析 ===")
        for diff in ['Easy', 'Medium', 'Hard']:
            diff_results = [r for r in tester.results if r['difficulty'] == diff]
            if diff_results:
